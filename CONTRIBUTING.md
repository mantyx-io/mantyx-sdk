...
# 3. Regenerate the CHANGELOGs from git history (commits since the last tag
#    stay under [Unreleased] until Publish creates tags — still required so
#    `changelog.mjs --check` passes on the branch).
...
5. Regenerate the three `CHANGELOG.md` files (now that tags exist) and push a `chore(repo): sync CHANGELOGs after v… release` commit to the branch.
6. Open a GitHub Release per tag, with notes generated by `scripts/changelog.mjs --release-body`.
