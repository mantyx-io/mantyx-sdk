...
- **Constrain replies to JSON** — pass an [`outputSchema`](/docs/output-schema/) (a JSON Schema), opt into strict enforcement when provider fallback must fail, and decode the result with the SDK's `parseRunOutput` helper.
- **Plan before you execute** — use [`runPlan`](/docs/planning/) for plan-only runs that return a structured checklist on `result.plan`, or pass `plan: true` on a normal run to track step statuses live via `task_plan` events.
